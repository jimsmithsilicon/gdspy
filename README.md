...
--------------------
16/6/20  Fork to add option to  read_gds, given a hash  for simultanous   gds_number/type pair, it can replace with a new gds_number /type  pair
A foundry had multiple layers on same  gds  number,  with different datatypes, which had to be split out onto separate gds layer numbers for processing with existing tools.

Changed library.py.

Usage example:
To convert file  sky.gds  to  a_well_known_Taiwan_foundry.gds  (0.18u typical gds numbers)

see 
example_layer_datatype_usage.py
